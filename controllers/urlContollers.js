...
try {
    const {url,createdBy,tag,deadline} = req.body
    console.log(url)
  ...
  if(urlValidator(url)){
        ...
        const newUrl = new Url({longUrl:url,urlId:urlId, shortUrl:shortUrl,createdBy:createdBy?createdBy:null, createdAt:Date.now(),tag:tag,deadline:deadline})
        await newUrl.save()
